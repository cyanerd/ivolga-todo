...
Дублирует пп. 15а и 15в. Фикс уже сделан в `order_ajax.js` (функция `editTotalBlock`): убрана вторая цена (`showPriceWithoutDiscount`), добавлен подсчёт количества из `GRID.ROWS` с русским склонением.
---

## 25. История заказов — не отображаются предыдущие заказы

**Статус:** Готово (24.02, фикс auth — нужно объединить старые аккаунты)
...
**Анализ (телефон +7 927 723 4706):** Найдено **11 аккаунтов** и **14 заказов**, раскиданных по разным user ID. Две причины:
1. **auth.php** искал по точному совпадению `PERSONAL_PHONE`. Пользователи из Mindbox (телефон `+7 (927) 723-47-06`) не находились при поиске по `79277234706` → создавался новый аккаунт
2. **Mindbox** при каждой операции создаёт нового пользователя вместо поиска существующего

**Решение (сайт):**
- `auth.php` и `check.php` — поиск по последним 10 цифрам телефона (`%PERSONAL_PHONE`) вместо точного совпадения. Сортировка по `ID ASC` — авторизует самый старый аккаунт (с историей заказов)
- `auth.php` — при обновлении кода нормализует `PERSONAL_PHONE` у найденного пользователя, чтобы формат стал единым

**Что ещё нужно:**
- В админке объединить заказы с дублей (ID 199, 201, 214, 233, 238, 253, 282, 296, 297, 301) на основной аккаунт (ID 175) и деактивировать дубли
- Разобраться с Mindbox — почему создаёт новых пользователей вместо поиска по телефону

---

## 26. Баннер cookie (152-ФЗ)
...
Добавлен баннер о согласии на использование cookie в соответствии с 152-ФЗ. Фиксированная плашка внизу экрана с текстом и ссылкой на политику конфиденциальности (`/info/policy/`). После нажатия «Принять» скрывается навсегда (localStorage).

**Решение:** HTML + JS в `header.php`, CSS в `app.css`. Адаптивный: на мобильных кнопка на всю ширину.
...
| 25 | История заказов не отображается | Готово (24.02, фикс auth) | Видит только заказы текущей сессии |
| 26 | Баннер cookie (152-ФЗ) | Готово (24.02) | — |
...
| 26 | Баннер cookie (152-ФЗ) | 1ч | Баннер согласия на cookie, адаптив, localStorage |
