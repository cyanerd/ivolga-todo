...
class MyTools {
    ...
    static public function getUserByFilter(array $arFilter, bool $only_published = false) {
        if($only_published)$arFilter['ACTIVE'] = 'Y'; $by="ID"; $order="DESC";
        $rsUsers = CUser::GetList(($by), ($order), $arFilter, array("SELECT"=>array("UF_*")));
    ...
    */
    static public function addPropetyToOrder( $ORDER_ID, $ORDER_PROPS_ID, $NAME, $VALUE ){
        $rsVals = CSaleOrderPropsValue::GetList(array("SORT" => "ASC"), array("ORDER_ID" => $ORDER_ID, "ORDER_PROPS_ID" => $ORDER_PROPS_ID));
    ...
    static public function isFavourite($item_id)
    {
        ...
        $arFilter = array('IBLOCK_ID' => 24, 'ACTIVE' => 'Y', '=PROPERTY_SESSION_ID' => $session_id, '=PROPERTY_ITEM_ID' => $item_id);
    ...
    static public function getColor($color) {
        if(empty(self::$arColors)) {
            ...
            $rsData = $entity_data_class::getList(array(
                'select' => array('UF_NAME', 'UF_COLOR_FILE'),
                'order' => array('ID' => 'ASC'),
            ));
    ...
    static public function getBasketBodyModal($basket, $format, $delivery_id = 0) {
        ...
        foreach ($basket['BASKET_ITEMS'] as $arItem):
            ...
            $rsElement = \CIBlockElement::GetList([], ['ACTIVE' => 'Y', '=ID' => $product_id], false, array(), ['ID', 'NAME', 'CATALOG_PRICE_7',]);
