...
foreach ($productIds as $productId) {
    ...
    if ($arElement = $rsElement->GetNext()) {
        ...
        $rsProps = CIBlockElement::GetProperty($arElement['IBLOCK_ID'], $productId, array("sort" => "asc"), array("CODE" => "MORE_PHOTO"));
        $arProps = array();
        ...
        $rsPropsNew = CIBlockElement::GetProperty($arElement['IBLOCK_ID'], $productId, array("sort" => "asc"), array("CODE" => "NEW"));
        if ($arProp = $rsPropsNew->GetNext()) {
            $arProps['NEW'] = $arProp;
        } else {
            $arProps['NEW'] = array('VALUE' => 'N');
        }
        
        $rsPropsPreorder = CIBlockElement::GetProperty($arElement['IBLOCK_ID'], $productId, array("sort" => "asc"), array("CODE" => "PREORDER"));
        if ($arProp = $rsPropsPreorder->GetNext()) {
            $arProps['PREORDER'] = $arProp;
        } else {
            $arProps['PREORDER'] = array('VALUE' => 'N');
        }
        
        $rsPropsTsvet = CIBlockElement::GetProperty($arElement['IBLOCK_ID'], $productId, array("sort" => "asc"), array("CODE" => "TSVET"));
        if ($arProp = $rsPropsTsvet->GetNext()) {
            $arProps['TSVET'] = $arProp;
        } else {
            $arProps['TSVET'] = array('VALUE' => '');
        }
        ...
        if ($arInfo) {
            $rsOffers = CIBlockElement::GetList(array(), array('IBLOCK_ID' => $arInfo['IBLOCK_ID'], 'PROPERTY_'.$arInfo['SKU_PROPERTY_ID'] => $productId), false, false, array("ID", "IBLOCK_ID", "NAME", "PRICE_7"));
        ...
        $rsPropsArticle = CIBlockElement::GetProperty($arElement['IBLOCK_ID'], $productId, array("sort" => "asc"), array("CODE" => "CML2_ARTICLE"));
        $article = '';
        ...
        // Получаем цвета
        $arColors = [];
        if (isset($arProps['TSVET']['VALUE']) && $arProps['TSVET']['VALUE'] && $article && function_exists('MyTools::getVariantColors')) {
            try {
        ...
        $arSizes = [];
        if ($arInfo) {
            $rsOffers = CIBlockElement::GetList(array(), array('IBLOCK_ID' => $arInfo['IBLOCK_ID'], 'PROPERTY_'.$arInfo['SKU_PROPERTY_ID'] => $productId), false, false, array("ID", "IBLOCK_ID", "NAME", "PROPERTY_RAZMER"));
