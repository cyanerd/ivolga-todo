...
//        "SECTION_CODE" => $_REQUEST["SECTION_CODE"],
  ...
  //      // Категория — фильтруем по SECTION_ID (ищем ID по названию)
  //      $catName = $_GET['category'];
  //      $secRes = CIBlockSection::GetList([], ['IBLOCK_ID' => 29, 'NAME' => $catName], false, ['ID']);
  //      if ($sec = $secRes->Fetch()) {
  //          $sectionId = $sec['ID'];
  //          $GLOBALS['arrFilter']['SECTION_ID'] = $sec['ID'];
  //      }
  //  }
  if (!empty($_GET['size'])) {
  ...
  <div id="catalog-ajax-container">
    <? $APPLICATION->IncludeComponent(
      ...
      [
        "IBLOCK_TYPE" => "CRM_PRODUCT_CATALOG",
        "IBLOCK_ID" => "29",
        // "SECTION_ID" => $_REQUEST["SECTION_ID"], // убрано для корректной фильтрации
        // "SECTION_CODE" => $_REQUEST["SECTION_CODE"], // убрано для корректной фильтрации
//      "SECTION_CODE_PATH" => $_REQUEST["SECTION_CODE_PATH"],
//      "ELEMENT_SORT_FIELD" => $_REQUEST["sort"] ?: "sort",
        "PRICE_CODE" => ["Розничная цена"],
//      "ELEMENT_SORT_ORDER" => $_REQUEST["order"] ?: "asc",
