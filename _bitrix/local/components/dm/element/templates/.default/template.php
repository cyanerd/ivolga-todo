    ...
    <div class="product-common">
        ...
        <? if (!empty($arResult['ITEM']['PROPERTIES']['MORE_PHOTO']['VALUE'])): ?>
          <div class="swiper-wrapper">
            <? foreach ($arResult['ITEM']['PROPERTIES']['MORE_PHOTO']['VALUE'] as $picture_id): ?>
              <?
              $img = CFile::ResizeImageGet($picture_id, ['width' => 566, 'height' => 895], BX_RESIZE_IMAGE_PROPORTIONAL, true);
              ?>
      ...
      <div class="product-config">
        <div class="product-config__head">
          <p
            class="product-config__title"><?= $arResult['ITEM']['PROPERTIES']['NAIMENOVANIE_TOVARA_NA_SAYTE_ETIKETKE']['VALUE'] ?? $arResult['ITEM']['NAME'] ?>
          </p>
          <p id="price-block"><?= number_format($arResult['OFFER']['PRICE_7'], 0, ' ', ' ') ?>&nbsp;₽</p>
        </div>
        <div class="product-config__params">
          <div>
            <button class="product-config-detail-button">
          ...
          <div class="product-config__size">
            <ul class="product-config__size-list">
              <? foreach ($arResult['SIZES_LIST'] as $arSize): ?>
                <li><a data-id="<?= $arSize['id'] ?>" data-price="<?= $arSize['price'] ?> ₽" href="javascript:"
                       class="product-config__size-button<?= ($arSize['id'] == $arResult['OFFER']['ID']) ? ' active' : '' ?>"><?= $arSize['title'] ?></a>
                </li>
              <? endforeach; ?>
            </ul>
          ...
          <div class="product-config__colors">
            <ul class="product-config__colors-list">
              <? foreach ($arResult['COLORS_LIST'] as $color => $detail_page): ?>
                <li>
                  <a title="<?= $color ?>" href="<?= $detail_page ?>"
                     class="product-config__colors-link<? if ($color == $arResult['CURRENT_COLOR']) echo ' selected' ?>">
          ...
          <div class="product-config__container">
            ...
            <button data-id="<?= $arResult['OFFER']['ID'] ?>"
            ...
            <? if (!empty($arResult['ITEM']['PROPERTIES']['UKHOD']['VALUE'])) { ?>
              <li class="product-config__info-item">
                <button class="product-config__button">
                  ...
                  <div
                    class="product-config-details-button__details "><?= $arResult['ITEM']['PROPERTIES']['UKHOD']['VALUE'] ?></div>
                </button>
              </li>
            <? } ?>
            <li class="product-config__info-item">
