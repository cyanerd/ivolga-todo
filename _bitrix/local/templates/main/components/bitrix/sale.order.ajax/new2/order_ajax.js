  ...
  BX.Sale.OrderAjaxComponent = {
    ...
    editActiveBasketBlock: function (activeNodeMode) {
      ...
      if (this.initialized.basket) {
        ...
        basketTable = BX.create('ul', {props: {className: 'bx-soa-item-table'}});
        ...
        const button = BX.create('div', {
          html: `<span id="_order-change-basket" data-modal="cart" onclick="loadCartContent()" class="button js--modal">Изменить корзину</span>`
        });

        basketContent.appendChild(
          BX.create('DIV', {
            props: {className: 'order-aside__cart bx-soa-table-fade' }, // 'bx-soa-table-fade'},
            // children: [
            //   BX.create('DIV', {
    ...
    editCoupons: function (basketItemsNode) {
      var couponsList = this.getCouponsList(true),
        ...
        couponsBlock = BX.create('DIV', {
          props: {className: 'bx-soa-coupon-block order-promo__form'},
          children: [
            BX.create('DIV', {
              props: {className: 'bx-soa-coupon-input order-promo__group form-group'},
              children: [
                BX.create('INPUT', {
                  props: {
      ...
      basketItemsNode.appendChild(
        BX.create('DIV', {
          props: {className: 'bx-soa-coupon order-promo'},
          children: [
            // couponsLabel,
            couponsBlock
          ]
    ...
    editActiveRegionBlock: function (activeNodeMode) {
      ...
      if (this.initialized.region) {
        ...
        regionNodeCol = BX.create('DIV', {props: {className: ''}});
    ...
    getDeliveryLocationInput: function (node) {
      ...
      if (altProperty) {
        ...
        label = BX.create('LABEL', {
          ...
          props: {className: 'bx-soa-custom-label bx-soa-custom-label--hehe'},
          html: labelTextHtml
        });
    ...
    editPaySystemItems: function (paySystemNode) {
      ...
      var paySystemItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-item-container order-methods'}}),
        paySystemItemNode, i;

      const row = BX.create('DIV', {props: {className: 'order-methods__row'}})

      paySystemItemsContainer.appendChild(row);
      ...
      for (i = 0; i < this.paySystemPagination.currentPage.length; i++) {
        ...
        row.appendChild(paySystemItemNode);
      }
    ...
    createPaySystemItem: function (item) {
      ...
      if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
        title = BX.create('h2', {props: {className: 'bx-soa-pp-company-smalltitle'}, text: item.NAME});
      }
    ...
    editDeliveryItems: function (deliveryNode) {
      ...
      var deliveryItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-item-container order-methods'}}),
        deliveryItemNode, k;
      ...
      // <div className="order-controls__group w-100">
      //   <div className="order-controls__preprice">Цена доставки: 500 ₽</div>
      // </div>
      ...
      for (k = 0; k < this.deliveryPagination.currentPage.length; k++) {
        deliveryItemNode = this.createDeliveryItem(this.deliveryPagination.currentPage[k]);
        row.appendChild(deliveryItemNode);
      }
    ...
    editActivePickUpBlock: function (activeNodeMode) {
      ...
      if (this.initialized.pickup) {
        ...
        pickUpContent.appendChild(
          BX.create('DIV', {
            props: {className: 'bx_soa_pickup row order-methods__card'},
    ...
    editPickUpList: function (isNew) {
      ...
      if (!!isNew) {
        ...
        container.appendChild(
          BX.create('h2', {
            props: {className: ''}, // bx-soa-pickup-subTitle bx-soa-pickup-subTitle-2
            html: 'Адрес шоурума' // this.params.MESS_PICKUP_LIST
          })
        );
    ...
    editPropsItems: function (propsNode) {
      ...
      var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-customer order-controls'}}),
    ...
    getPropertyRowNode: function (property, propsItemsContainer, disabled) {
      ...
      if (disabled) {
        ...
        BX.addClass(propsItemNode, "form-group bx-soa-customer-field order-controls__group w-50");
        ...
        //   props: {className: 'bx-soa-custom-label bx-soa-custom-label--2'},
        //   html: textHtml
        // });
    ...
    insertStringProperty: function (property, propsItemNode, disabled) {
      ...
      if (disabled) {
        ...
        const label = BX.create('LABEL', {
          attrs: {'for': 'soa-property-' + property.getId()},
          props: {className: 'bx-soa-custom-label bx-soa-custom-label--2 form-label'},
          html: property.getName()
        });

        propContainer = BX.create('DIV', {props: {className: 'soa-property-container form-group form-group--3'}});
        property.appendTo(propContainer);
    ...
    editPropsComment: function (propsNode) {
      ...
      const wrapper = BX.create('DIV', {props: {className: 'order-controls _order-controls'}});

      propsCommentContainer = BX.create('DIV', {props: {className: 'form-group bx-soa-customer-field order-controls__group w-100 _order-comment-container'}});
      label = BX.create('LABEL', {
        attrs: {for: 'orderDescription'},
        props: {className: 'bx-soa-customer-label form-label'},
      ...
      wrapper.appendChild(propsCommentContainer);
      propsNode.appendChild(wrapper);
    },
    ...
    createTotalUnit: function (name, value, params) {
      ...
      return BX.create('tr', {
        ...
        children: [
          BX.create('td', {props: {className: 'bx-soa-cart-t'}, text: name}),
          BX.create('td', {
            props: {
    ...
    basketBlockScrollCheck: function () {
      ...
      for (i = 0; i < scrollableNodes.length; i++) {
        ...
        tableNode = parentNode.querySelector('ul.bx-soa-item-table');
