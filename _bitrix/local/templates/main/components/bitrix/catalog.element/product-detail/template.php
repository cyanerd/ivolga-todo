      ...
      <div class="pageprod__row">
        ...
        <div class="pageprod__right">
          ...
          <p class="pageprod__prices">
            ...
            $rsOffers = CIBlockElement::GetList(array(), array('IBLOCK_ID' => $arInfo['IBLOCK_ID'], 'PROPERTY_'.$arInfo['SKU_PROPERTY_ID'] => $arResult['ID']), false, false, array("ID", "IBLOCK_ID", "NAME", "PRICE_7"));
          ...
          <div class="pageprod__params prodpar">
            ...
            $arInfo = CCatalogSKU::GetInfoByProductIBlock($arResult["IBLOCK_ID"]);
            $rsOffers = CIBlockElement::GetList(array(), array('IBLOCK_ID' => $arInfo['IBLOCK_ID'], 'PROPERTY_'.$arInfo['SKU_PROPERTY_ID'] => $arResult['ID']), false, false, array("ID", "IBLOCK_ID", "NAME", "PROPERTY_RAZMER"));
            $sizes_list = [];
                    ...
                    <div class="product-card__price">
                      <div class="product-card__price-current">
                        ...
                        <? if ($relatedProduct["PRICES"]["BASE"]["DISCOUNT_DIFF"] > 0): ?>
                          <div class="product-card__discount">-<?= round(($relatedProduct["PRICES"]["BASE"]["DISCOUNT_DIFF"] / $relatedProduct["PRICES"]["BASE"]["VALUE"]) * 100) ?>%</div>
                        <? endif; ?>
                      </div>
                      <? if ($relatedProduct["PRICES"]["BASE"]["DISCOUNT_VALUE"]): ?>
                        <span class="product-card__price-old"><?= number_format($relatedProduct["PRICES"]["BASE"]["VALUE"], 0, '.', ' ') ?>₽</span>
                      <? endif; ?>
                    </div>
                  </a>
                    ...
                    <? if ($relatedProduct["PROPERTIES"]["COLORS"]["VALUE"]): ?>
                      <div class="product-card__colors">
                        <? foreach (array_slice($relatedProduct["PROPERTIES"]["COLORS"]["VALUE"], 0, 5) as $key => $color): ?>
                          <a href="<?= $relatedProduct["DETAIL_PAGE_URL"] ?>" class="product-card__colors-item" style="background-color: <?= $relatedProduct["PROPERTIES"]["COLORS"]["VALUE_XML_ID"][$key] ?: '#f6f5f3'; ?>;"></a>
                        <? endforeach; ?>
                      </div>
                    <? endif; ?>
